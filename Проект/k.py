...
Принцип игры
'''
canvas.create_text(300, 15, text=fa, fill="red", font=("Helvectica", "14"))
faq = '''
Компьютер загадывает слово — пишет на бумаге
...
то Компьютер пишет её над соответствующими этой
...
def arr():
    ...
    def a(v):
        ...
        if v in wor:
            ...
            if len(win) == 6:
                canvas.create_text(150, 150, text="Вы выиграли", fill="purple", font=("Helvectica", "16"))
                for i in alfabet:
...
btn01 = Button(root, text='Начать игру', width=15, height=1, command=lambda: arr())
